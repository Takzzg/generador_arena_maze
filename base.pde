...
class Robot {
  ...
  void recorrer() {
    ...
    if (!arena[robot.z][y][x].visited) {
      if (arena[robot.z][y][x].leftVictim)kits[y][x] = new Kit('L');
      if (arena[robot.z][y][x].rightVictim)kits[y][x] = new Kit('R');
    }
    if (!ignore) {
      switch (dir) {
    ...
    if (!ignore) {
      ...
      if (arena[robot.z][y][x].leftVictim) { 
        kits[y][x] = new Kit('L'); 
        stroke(255, 255, 0, 100); 
        strokeWeight(25); 
        point(x * example.wid, y * example.wid + example.wid/2); 
        frameRate(2);
      }
      if (arena[robot.z][y][x].rightVictim) { 
        kits[y][x] = new Kit('R'); 
        stroke(255, 0, 255, 100); 
        strokeWeight(25); 
        point(x * example.wid + example.wid, y * example.wid + example.wid/2); 
        frameRate(2);
      }
    }
  }
